...
The onboarding screen uses a **fullscreen background image** with a gradient overlay. Use a **local asset** (`require("@/assets/...")`). Slides are driven by a `FlatList` with `pagingEnabled`. Fires `Analytics` events on each slide view and on completion.
...
const SLIDES = [
  { key: "1", titleKey: "onboarding.slide1.title", descKey: "onboarding.slide1.description", icon: "history-edu" },
  { key: "2", titleKey: "onboarding.slide2.title", descKey: "onboarding.slide2.description", icon: "category" },
  { key: "3", titleKey: "onboarding.slide3.title", descKey: "onboarding.slide3.description", icon: "calendar-today" },
  { key: "4", titleKey: "onboarding.slide4.title", descKey: "onboarding.slide4.description", icon: "workspace-premium" },
];
...
export default function OnboardingScreen() {
  ...
  useEffect(() => {
    Analytics.logScreenView("OnboardingScreen");
    Analytics.logOnboardingSlideView(0);
  }, []);
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <FlatList
          ...
          onMomentumScrollEnd={(e) => {
            ...
            Analytics.logOnboardingSlideView(index);
          }}
          renderItem={({ item }) => (
            <View style={styles.slide}>
              <View style={styles.iconCircle}>
                <MaterialIcons name={item.icon as any} size={52} color="#FFFFFF" />
              </View>
...
const styles = StyleSheet.create({
  ...
  iconCircle: { width: 96, height: 96, borderRadius: 48, alignItems: "center", justifyContent: "center", marginBottom: 32, backgroundColor: "rgba(65,114,157,0.35)", borderWidth: 1.5, borderColor: "rgba(65,114,157,0.6)" },
  ...
  slideTitle: { fontSize: 36, fontWeight: "700", color: "#FFFFFF", textAlign: "center", marginBottom: 16 },
  slideDesc: { fontSize: 17, color: "rgba(255,255,255,0.75)", textAlign: "center" },
  ...
  ctaContainer: { paddingHorizontal: 24, paddingBottom: 40 },
  ctaButton: { width: "100%", backgroundColor: "#6C63FF", borderRadius: 16, alignItems: "center", paddingVertical: 16 },
  ctaButtonText: { color: "#FFFFFF", fontSize: 18, fontWeight: "700" },
});
```
...
> - `testID="skip-button"` and `testID="get-started-button"` required for Maestro tests
...
- Interstitial fires only every 3rd action (`INTERSTITIAL_QUIZ_INTERVAL = 3`)
- Interstitial cooldown is 5 minutes
- Rewarded ad is always loaded — even premium users can use jokers/hints
- Two-layer first-launch guard: `isFirstLaunchRef` (cold start) + `LS_FIRST_LAUNCH_DONE_KEY` (install)
- `addAdEventListener` — NOT `addEventHandler`
...
Call `showInterstitial()` from `useAds()` **after** a meaningful user action. Cooldown (5 min) and quiz interval gate (`INTERSTITIAL_QUIZ_INTERVAL = 3`) are enforced automatically — just call it freely at good breakpoints.
...
Add all ad unit IDs to `ADMOB_UNITS` in `src/constants/urls.ts`:

```ts
export const ADMOB_UNITS = {
  banner:       { ios: "ca-app-pub-xxx/BANNER_IOS",       android: "ca-app-pub-xxx/BANNER_ANDROID" },
  interstitial: { ios: "ca-app-pub-xxx/INTER_IOS",        android: "ca-app-pub-xxx/INTER_ANDROID" },
  rewarded:     { ios: "ca-app-pub-xxx/REWARDED_IOS",     android: "ca-app-pub-xxx/REWARDED_ANDROID" },
  appOpen:      { ios: "ca-app-pub-xxx/APP_OPEN_IOS",     android: "ca-app-pub-xxx/APP_OPEN_ANDROID" },
  native:       { ios: "ca-app-pub-xxx/NATIVE_IOS",       android: "ca-app-pub-xxx/NATIVE_ANDROID" },
};
```

| Format       | AdMob Console Location            |
| ------------ | --------------------------------- |
...
- `shouldShowAds = !isPremium && !purchasesLoading` — ads blocked while loading and for premium users
- Rewarded always loads — even for premium users (jokers/hints)
...
export function PurchasesProvider({ children }: { children: React.ReactNode }) {
  ...
  // Fetch subscription products once IAP is connected
  useEffect(() => {
    if (connected) {
      fetchProducts({ skus: SUBSCRIPTION_SKUS, type: "subs" }).then(() => {
      ...
      if (hasPremium) {
        ...
        for (const p of activeSubs) {
          ...
          if (expMs) {
            ...
            bestProductId = p.productId; // Android: no expirationDate field
          }
        }
  ...
  // Check premium on startup
  useEffect(() => {
    refreshPremiumStatus();
  }, [refreshPremiumStatus]);

  // DEV-only: toggle premium for testing (never in production builds)
...
>
> - SKUs come from `urls.ts` — never hardcode here
> - `productsLoaded` is used by paywall for shimmer skeleton animation
> - `subscriptions` is forwarded from `useIAP` so paywall can access price data directly
> - `drainPendingTransactions` acknowledges unfinished transactions on startup
> - `premiumExpiryDate` is iOS only (`expirationDateIOS`); Android doesn't expose this field
> - `debugTogglePremium` is `__DEV__` only — not included in production builds
...
- Interstitial: 5-minute cooldown + every-3-quizzes interval gate — enforced automatically via `localStorage`
